...
func Setup(steamPath string, appInfo *models.SteamAppInfo, manifests []models.ManifestInfo) error {
	...
	fmt.Fprintf(&b, "-- Generated Lua Manifest by SteamToolbox\n")
	...
	// Emit addappid + setManifestid (SteamTools requires both for manifest pinning).
	...
	for _, m := range manifests {
		...
		if m.ManifestID != "" {
			fmt.Fprintf(&b, "setManifestid(%s, \"%s\", 0)\n", m.DepotID, m.ManifestID)
		}
	}
